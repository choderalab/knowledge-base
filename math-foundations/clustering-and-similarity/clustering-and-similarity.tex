...
\subsubsection{Proving that the Tanimoto Coefficient is a Special Case of the Tversky Index}
\[|A \backslash B| = |A| - |A\cap B|\]
\[|B \backslash A| = |B| - |A\cap B|\]
\[Let \alpha = \beta = 1\]
\[TI(A,B) = \frac{|A\cap B|}{|A \backslash  B| + |B \backslash A| + |A\cap B|}\]
\[TI(A,B) = \frac{|A\cap B|}{|A| - |A\cap B| + |B| - |A\cap B| + |A\cap B|}\]
\[TI(A,B) = \frac{|A\cap B|}{|A| + |B| - |A\cap B|}\]
\[TI(A,B) = TC(A,B)\]

\subsubsection{Proving that the Dice-Sørensen Coefficient is a Special Case of the Tversky Index}
\[Let \alpha = \beta = 0.5\]
\[TI(A,B) = \frac{|A\cap B|}{0.5|A \backslash  B| + 0.5|B \backslash A| + |A\cap B|}\]
\[TI(A,B) = \frac{|A\cap B|}{0.5(|A| - |A\cap B|) + 0.5(|B| - |A\cap B|) + |A\cap B|}\]
\[TI(A,B) = \frac{|A\cap B|}{0.5|A| - 0.5|A\cap B| + 0.5|B| - 0.5|A\cap B| + |A\cap B|}\]
\[TI(A,B) = \frac{2|A\cap B|}{|A| + |B|}\]
\[TI(A,B) = DC(A,B)\]
